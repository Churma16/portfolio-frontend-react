...
import { HiXMark, HiOutlineGlobeAlt, HiOutlineCodeBracket } from "react-icons/hi2";
                        ...
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex gap-3">
                                {selectedProject.repo_url && (
                                    <a href={selectedProject.repo_url} target="_blank" rel="noreferrer" className="p-2 text-slate-400 hover:text-white bg-white/5 rounded-lg border border-white/5 hover:border-white/20 transition-all">
                                        <HiOutlineCodeBracket className="w-6 h-6" />
                                    </a>
                                )}
                                {selectedProject.demo_url && (
                                     <a href={selectedProject.demo_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-lara-blue text-white rounded-lg font-bold text-sm hover:bg-blue-600 transition-colors shadow-[0_0_15px_rgba(59,130,246,0.4)]">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {selectedProject.tech_stack?.map((stack, idx) => (
                                    <div key={idx} className="flex items-center gap-2 bg-white/5 border border-white/5 px-3 py-2 rounded-lg">
                                        <TechIcon name={stack.name} className="w-5 h-5 text-lara-blue" />
